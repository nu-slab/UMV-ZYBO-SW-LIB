  ...
  pcam5c::pcam5c(const std::string& iic_devname, OV5640_cfg::mode_t mode, OV5640_cfg::awb_t  awb):
	...
	void pcam5c::work(OV5640_cfg::mode_t mode) const
	{
		...
		std::cout << std::endl << "### Bring up input pipeline back-to-front ###" << std::endl;
		{
			...
			MIPI_CSI_2_RX_mWriteReg(XPAR_SYSTEM_PCAMINTERFACE_MIPI_CSI_2_RX_0_S_AXI_LITE_BASEADDR, CR_OFFSET, (CR_RESET_MASK & ~CR_ENABLE_MASK));
			MIPI_D_PHY_RX_mWriteReg(XPAR_SYSTEM_PCAMINTERFACE_MIPI_D_PHY_RX_0_S_AXI_LITE_BASEADDR, CR_OFFSET, (CR_RESET_MASK & ~CR_ENABLE_MASK));
			reset();
		}
		{
		...
		}
		{
			...
			MIPI_CSI_2_RX_mWriteReg(XPAR_SYSTEM_PCAMINTERFACE_MIPI_CSI_2_RX_0_S_AXI_LITE_BASEADDR, CR_OFFSET, CR_ENABLE_MASK);
			MIPI_D_PHY_RX_mWriteReg(XPAR_SYSTEM_PCAMINTERFACE_MIPI_D_PHY_RX_0_S_AXI_LITE_BASEADDR, CR_OFFSET, CR_ENABLE_MASK);
			set_mode(mode);
