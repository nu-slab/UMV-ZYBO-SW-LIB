...
namespace digilent
{
	namespace OV5640_cfg
	{
		...
		typedef enum   { MODE_720P_1280_720_60fps = 0, MODE_1080P_1920_1080_15fps,
			MODE_1080P_1920_1080_30fps, MODE_VGA_640_480_90fps, MODE_END } mode_t;
		typedef struct { uint16_t width; uint16_t height; } mode_resolution_t;

		/* 
		 * Those tables below are provided by digilent et al.
		 * For further configulation or modification, you may want to refer to below link (OV5640 datasheet)
		...
		config_word_t const cfg_VGA_90fps_[] =
		{
		...
		config_modes_t const modes[] =
		{
			...
			{MAP_ENUM_TO_CFG(MODE_VGA_640_480_90fps,     cfg_VGA_90fps_  )}
		};
	...
	class pcam5c {
		private:
			...
			std::unique_ptr<digilent::PS_GPIO> psgpio_;
			std::unique_ptr<digilent::PS_IIC>  psiic_;
			uint8_t* v4l2_buf_;
			uint32_t width_, height_;
		protected:
		public:
			pcam5c(const std::string& iic_devname,
			...
			bool set_mode(digilent::OV5640_cfg::mode_t&) const;
			bool set_awb(digilent::OV5640_cfg::awb_t) const;
			void work(OV5640_cfg::mode_t mode) const;
	};
